...
def view_note():
    list_notes()
    try:
        index = int(input("\nEnter note number to view: ").strip()) - 1
        data = load_vault()
        if 0 <= index < len(data):
...
def search_notes():
    keyword = input("Enter keyword used in title to search notes : ").strip().lower()
    data = load_vault()
    found = False
    result = []
    ...
    if len(result) >= 1 and found == True:
        print("\nMatching results are: ")
        for indx, note in enumerate(result):
            print(f"{indx}. {note['title']} - {note['timestamp']}")
